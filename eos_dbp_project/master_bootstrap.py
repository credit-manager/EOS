...
class DBPField(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    entity_id = Column(String(36), ForeignKey("dbp_entities.id", ondelete="CASCADE"), nullable=False, index=True)
...
# ─── setup_db.py ─── (هذا هو الملف الذي كان يسبب المشكلة)
...
# 1. إنشاء جميع الجداول تلقائياً
print("🔄 Creating tables...")
Base.metadata.create_all(bind=engine)
print("✅ Tables created!")

# 2. إدخال بيانات تجريبية (كيان الحسابات)
print("🔄 Seeding data...")
db = SessionLocal()
try:
    # استخدام جدول tenants المضمون الوجود كـ table_mapping
    db.execute(text(\"\"\"
    ...
    # جلب ID الكيان
    result = db.execute(text("SELECT id FROM dbp_entities WHERE code = 'account'")).fetchone()
    if result:
        eid = result[0]
        db.execute(text(\"\"\"
            INSERT INTO dbp_fields (entity_id, code, label_en, label_ar, field_type, is_required, ui_config)
            VALUES
            (:eid, 'code', 'Code', 'الكود', 'string', true, '{"component": "input", "order": 1}'),
            (:eid, 'name', 'Name', 'الاسم', 'string', true, '{"component": "input", "order": 2}'),
            (:eid, 'account_type', 'Type', 'النوع', 'enum', true, '{"component": "select", "order": 3}')
...
class MetadataEngine:
    ...
    def get_entity_by_code(self, code: str) -> Optional[Dict[str, Any]]:
        entity = self.db.query(DBPEntity).filter(DBPEntity.code == code).first()
        if not entity:
            return None
        return {k: v for k, v in entity.__dict__.items() if k != '_sa_instance_state'}

    def get_entity_fields(self, entity_id: str) -> list:
        fields = self.db.query(DBPField).filter(DBPField.entity_id == entity_id).order_by(
            text("CAST(ui_config->>'order' AS INTEGER) ASC")
        ).all()
        return [{k: v for k, v in f.__dict__.items() if k != '_sa_instance_state'} for f in fields]
    ...
    def validate_data(self, code: str, data: Dict[str, Any]):
...
async def get_schema(entity_code: str, db: Session = Depends(get_db)):
    try:
        return {"status": "success", "data": MetadataEngine(db).get_full_schema(entity_code)}
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))

@router.get("/entities/{entity_code}/records")
async def list_records(entity_code: str, limit: int = 100, db: Session = Depends(get_db)):
    engine = MetadataEngine(db)
    entity = engine.get_entity_by_code(entity_code)
    if not entity or not entity.get("table_mapping"):
        raise HTTPException(status_code=404, detail="الكيان أو الجدول غير موجود")
    query = text(f"SELECT * FROM {entity['table_mapping']} LIMIT :limit")
    result = db.execute(query, {"limit": limit})
    return {"status": "success", "data": [dict(row._mapping) for row in result], "count": result.rowcount}

@router.post("/entities/{entity_code}/records")
async def create_record(entity_code: str, payload: Dict[str, Any], db: Session = Depends(get_db)):
    engine = MetadataEngine(db)
    try:
    ...
    query = text(f"INSERT INTO {entity['table_mapping']} (id, tenant_id, {cols}) VALUES (:id, :tenant_id, {placeholders}) RETURNING id")
    try:
...
app = FastAPI(title="EOS Dynamic Business Platform", version="1.0.0")
...
try:
    ...
    print("⚠️ تأكد من أن PostgreSQL يعمل وأن بيانات الاتصال صحيحة في database.py")
    sys.exit(1)
...
try:
    ...
    print("\n📋 Test 1: Get Schema")
    try:
        with urllib.request.urlopen("http://127.0.0.1:8000/api/v1/dynamic/entities/account/schema") as response:
            data = json.loads(response.read().decode())
            print(f"✅ Success: {json.dumps(data, ensure_ascii=False, indent=2)}")
    except Exception as e:
        print(f"❌ Failed: {e}")

    # اختبار 2: سرد السجلات
    print("\n📋 Test 2: List Records")
    try:
        with urllib.request.urlopen("http://127.0.0.1:8000/api/v1/dynamic/entities/account/records?limit=5") as response:
            data = json.loads(response.read().decode())
    ...
    try:
        req = urllib.request.Request(
            ...
            data=json.dumps({"code": "1000-01", "name": "Test Account", "account_type": "asset"}).encode(),
            headers={"Content-Type": "application/json"},
            method="POST"
        )
        with urllib.request.urlopen(req) as response:
            data = json.loads(response.read().decode())
            print(f"✅ Success: Created record with ID {data.get('id')}")
    except Exception as e:
        print(f"❌ Failed: {e}")
