...
def _create_published_entity(headers: dict[str, str], code: str) -> None:
    response = client.post(
...
def test_admin_can_query_own_tenant_audit_events() -> None:
    ...
    code = f"audit_entity_{uuid4().hex[:8]}"
    ...
    assert any(event["action"] == "metadata.created" and event["resource_type"] == code for event in payload)
...
def test_audit_query_filters_by_action_and_limit() -> None:
    ...
    headers = _headers(session)
    code = f"filter_entity_{uuid4().hex[:8]}"
    _create_published_entity(headers, code)
    response = client.get("/api/v1/audit/events?action=metadata.created&limit=2", headers=headers)
    assert response.status_code == 200
    assert len(response.json()) <= 2
    assert all(event["action"] == "metadata.created" for event in response.json())
...
def test_audit_query_cannot_read_another_tenant() -> None:
    ...
    _create_published_entity(_headers(first), f"first_entity_{uuid4().hex[:8]}")
    _create_published_entity(_headers(second), f"second_entity_{uuid4().hex[:8]}")
