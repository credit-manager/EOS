...
def balance_sheet(
    ...
    total_liabilities = sum((x["amount"] for x in liabilities), _ZERO)
    total_equity = sum((x["amount"] for x in equity), _ZERO)
    return {
        "currency": currency,
...
def income_statement(
    ...
    for b in balances:
        if b["account_type"] == "revenue":
            ...
            revenue_lines.append({"account_id": b["account_id"], "code": b["code"], "name": b["name"], "amount": amount})
        elif b["account_type"] == "expense":
            expense_lines.append({"account_id": b["account_id"], "code": b["code"], "name": b["name"], "amount": b["balance"]})
...
def cash_flow(
    ...
    for b in balances:
        line = {"account_id": b["account_id"], "code": b["code"], "name": b["name"], "amount": b["balance"]}
        if b["account_type"] in {"revenue", "expense"}:
            operating.append(line)
        elif b["account_type"] == "asset":
            (operating if b["code"].startswith("1") else investing).append(line)
        elif b["account_type"] in {"liability", "equity"}:
            financing.append(line)
    total_operating = sum((x["amount"] for x in operating), _ZERO)
    total_investing = sum((x["amount"] for x in investing), _ZERO)
    total_financing = sum((x["amount"] for x in financing), _ZERO)
    return {
        "currency": currency,
        "period_start": period_start.isoformat() if period_start else None,
        "period_end": period_end.isoformat() if period_end else None,
