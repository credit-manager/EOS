...
def upgrade():
    ...
    BEGIN
      FOR r IN
        ...
        WHERE t.relname = 'dbp_entities' AND c.contype = 'u' AND a.attname = 'code'
      LOOP
    ...
    BEGIN
      FOR r IN
        ...
        WHERE schemaname='public' AND tablename='dbp_entities'
          AND indexdef ILIKE '%UNIQUE%' AND indexdef ILIKE '%(code)%'
      LOOP
    ...
    op.execute("""
      ...
      ON public.dbp_entities (tenant_id, code) WHERE tenant_id IS NOT NULL
    """)
    op.execute("""
      CREATE UNIQUE INDEX IF NOT EXISTS uq_dbp_entities_global_code
      ON public.dbp_entities (code) WHERE tenant_id IS NULL
    """)
