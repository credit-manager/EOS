"""Harden builder DDL for already-migrated production databases.

Revision ID: 20260910_builder_ddl_hardening
Revises: 20260909_builder_ddl_security
"""
...
revision = "20260910_builder_ddl_hardening"
down_revision = "20260909_builder_ddl_security"
branch_labels = None
depends_on = None
...
BEGIN
    ...
    LOOP
        IF jsonb_typeof(col.value) <> 'object'
           OR NOT (col.value ? 'code')
           OR NOT (col.value ? 'sql_type') THEN
        ...
        IF v_type NOT IN (
            'VARCHAR(255)','TEXT','INTEGER','DOUBLE PRECISION','BOOLEAN',
            'DATE','TIMESTAMP','VARCHAR(50)','JSONB'
        ) THEN
