...
**Implementation update (current branch):** the browser entrypoint now delegates to one canonical `PlatformApp` runtime. It uses the metadata catalog/entity/studio path, a shared API client, hash routes for workspace/object/builder navigation, the analytics home feed, refresh rotation, and server logout. Workflow/rules/events/graph/AI views and the full demo chain remain incomplete.
