...
def check_permission(user: dict, action: str):
    ...
    if "platform_owner" in roles or "admin" in roles:
        return True
    if any(role in PERMISSION_MATRIX.get(action, []) for role in roles):
        return True
...
def audit_log(db: Session, tenant_id: str, user_id: str, action: str,
    ...
    db.execute(
        ...
        {"id": uid(), "tid": tenant_id, "uid": user_id, "act": action, "et": entity_type,
         "eid": entity_id, "old": json.dumps(old_values or {}), "new": json.dumps(new_values or {}),
         "now": now()},
    )
...
def post_journal(db: Session, tenant_id: str, company_id: str,
    ...
    """Post a balanced journal and update the tenant's GL accounts atomically."""
    if not lines:
        raise HTTPException(400, detail="Journal must contain at least one line")
    ...
    db.execute(
        ...
        {"id": jid, "tid": tenant_id, "cid": company_id, "num": entry_number,
    ...
    for i, line in enumerate(lines):
        ...
        if not account_code:
            raise HTTPException(400, detail="Journal line account_code is required")

        account = db.execute(text(
            "SELECT id FROM dbp_accounts "
            "WHERE code=:code AND tenant_id=:tid AND company_id=:cid AND is_active=true"
        ), {"code": account_code, "tid": tenant_id, "cid": company_id}).fetchone()
        if not account:
            raise HTTPException(400, detail=f"Account not found for tenant/company: {account_code}")
        ...
        db.execute(
            ...
            {"id": lid, "jid": jid, "acct": account[0], "desc": line.get("description", ""),
             "dr": line.get("debit", 0), "cr": line.get("credit", 0),
             "cc": line.get("cost_center_id") or line.get("cost_center"),
             "ord": i + 1, "now": now()},
        )
        ...
        db.execute(
            text("UPDATE dbp_accounts SET current_balance = current_balance + :dr - :cr "
                 "WHERE id=:aid AND tenant_id=:tid AND company_id=:cid"),
            {"aid": account[0], "dr": dr, "cr": cr, "tid": tenant_id, "cid": company_id},
        )
    return jid
...
def atomic_stock_receive(db: Session, tenant_id: str, item_id: str,
    ...
    if existing:
        ...
        new_unit_cost = ((old_qty * old_cost) + total_cost) / new_qty if new_qty else Decimal("0")
        db.execute(text(f"UPDATE {stock_table} SET on_hand=:q, unit_cost=:uc WHERE id=:sid"),
                   {"q": new_qty, "uc": new_unit_cost, "sid": existing[0]})
        return existing[0], float(new_unit_cost)
    sid = uid()
    db.execute(text(f"INSERT INTO {stock_table} (id, tenant_id, {item_column}, warehouse_id, on_hand, unit_cost) "
                    "VALUES (:id,:t,:ic,:w,:q,:uc)"),
