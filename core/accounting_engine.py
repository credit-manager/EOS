"""P22 Accounting Engine — tenant-safe double-entry bookkeeping."""
import uuid
...
class AccountingEngine:
    ...
    def create_account(self, tenant_id: str, company_id: str, code: str, name_en: str, account_type: str, **kw) -> str | None:
        ...
        if parent_id and not self.db.execute(text("SELECT id FROM dbp_accounts WHERE id=:pid AND tenant_id=:tid AND company_id=:cid"), {"pid": parent_id, "tid": tenant_id, "cid": company_id}).fetchone(): return None
    ...
    def post_journal_entry(self, je_id: str, tenant_id: str) -> dict[str, Any]:
        entry=self.db.execute(text("SELECT id,status,company_id FROM dbp_journal_entries WHERE id=:jid AND tenant_id=:t FOR UPDATE"), {"jid":je_id,"t":tenant_id}).fetchone()
    ...
    def list_journal_entries(self, company_id: str, tenant_id: str, status: str | None = None, limit: int = 50, offset: int = 0) -> list[dict]:
        limit=max(1,min(int(limit),500)); offset=max(0,int(offset)); conditions=["company_id=:cid","tenant_id=:t"]; params={"cid":company_id,"t":tenant_id,"lim":limit,"off":offset}
