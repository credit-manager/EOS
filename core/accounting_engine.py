...
class AccountingEngine:
    ...
    def _get_journal_company(self, journal_entry_id: str, tenant_id: str) -> str:
        """Return the journal company after enforcing tenant ownership."""
        row = self.db.execute(text(
            "SELECT company_id FROM dbp_journal_entries "
            "WHERE id = :jid AND tenant_id = :tenant_id"
        ), {"jid": journal_entry_id, "tenant_id": tenant_id}).fetchone()
        if not row:
            from fastapi import HTTPException
    ...
    def _verify_account_scope(self, account_id: str, tenant_id: str, company_id: str):
        """Raise 403 unless the account belongs to the same tenant and company."""
        row = self.db.execute(text(
            "SELECT company_id FROM dbp_accounts "
            "WHERE id = :aid AND tenant_id = :tenant_id"
        ), {"aid": account_id, "tenant_id": tenant_id}).fetchone()
    ...
    def create_journal_entry(self, tenant_id: str, company_id: str,
        ...
        ), {"id": jeid, "tid": tenant_id, "cid": company_id, "fyid": fiscal_year_id, "en": entry_number, "ed": entry_date, "et": entry_type, "desc": description, "ref": reference, "cb": created_by})
        self.db.flush()
        return jeid
    ...
    def add_journal_line(self, journal_entry_id: str, account_id: str, tenant_id: str,
        ...
        company_id = self._get_journal_company(journal_entry_id, tenant_id)
        self._verify_account_scope(account_id, tenant_id, company_id)
        ...
        self.db.execute(text(
            "INSERT INTO dbp_journal_lines (id, journal_entry_id, account_id, debit, credit, description, cost_center_id, line_order) "
            "VALUES (:id, :jid, :aid, :dr, :cr, :desc, :ccid, :lo)"
        ), {"id": lid, "jid": journal_entry_id, "aid": account_id, "dr": debit, "cr": credit, "desc": description, "ccid": cost_center_id, "lo": max_order + 1})
        self.db.flush()
        return lid

    def post_journal_entry(self, je_id: str, tenant_id: str) -> dict[str, Any]:
        """Post a journal entry (validate balance and account company scope before GL updates)."""
        ...
        # Validate every account before changing any GL balance. This prevents
        # a partially-posted entry if one line references another company.
        for line in lines:
            self._verify_account_scope(line[1], tenant_id, entry[2])
        ...
        self.db.execute(text(
            "UPDATE dbp_journal_entries SET status='posted', is_posted=true, posted_at=NOW(), total_debit=:td, total_credit=:tc "
            "WHERE id = :jid AND tenant_id = :t AND company_id = :cid"
        ), {"jid": je_id, "t": tenant_id, "cid": entry[2], "td": total_debit, "tc": total_credit})
        self.db.flush()
        return {"success": True, "total_debit": float(total_debit), "total_credit": float(total_credit), "lines_count": len(lines)}

    def get_journal_entry(self, je_id: str, tenant_id: str) -> dict | None:
        r = self.db.execute(text(
            "SELECT id, entry_number, entry_date, entry_type, description, reference, status, total_debit, total_credit, is_posted, created_by, created_at "
            "FROM dbp_journal_entries WHERE id = :jid AND tenant_id = :t"
        ), {"jid": je_id, "t": tenant_id}).fetchone()
        if not r:
            return None
        lines = self.db.execute(text(
            "SELECT l.id, l.account_id, a.code, a.name_en, l.debit, l.credit, l.description, l.cost_center_id "
            "FROM dbp_journal_lines l LEFT JOIN dbp_accounts a ON l.account_id = a.id "
            "WHERE l.journal_entry_id = :jid ORDER BY l.line_order"
        ), {"jid": je_id}).fetchall()
        return {"id": r[0], "entry_number": r[1], "entry_date": str(r[2]) if r[2] else None, "entry_type": r[3], "description": r[4], "reference": r[5], "status": r[6], "total_debit": float(r[7]) if r[7] else 0, "total_credit": float(r[8]) if r[8] else 0, "is_posted": bool(r[9]), "created_by": r[10], "created_at": r[11].isoformat() if r[11] else None, "lines": [{"id": l[0], "account_id": l[1], "account_code": l[2], "account_name": l[3], "debit": float(l[4]) if l[4] else 0, "credit": float(l[5]) if l[5] else 0, "description": l[6], "cost_center_id": l[7]} for l in lines]}
    ...
    def list_journal_entries(self, company_id: str, tenant_id: str, status: str | None = None, limit: int = 50, offset: int = 0) -> list[dict]:
        ...
        rows = self.db.execute(text(f"SELECT id, entry_number, entry_date, entry_type, description, status, total_debit, total_credit, created_at FROM dbp_journal_entries WHERE {where} ORDER BY entry_date DESC, entry_number DESC LIMIT :lim OFFSET :off"), params).fetchall()
    ...
    def get_trial_balance(self, company_id: str, tenant_id: str) -> dict[str, Any]:
        accounts = self.db.execute(text(
            "SELECT id, code, name_en, account_type, current_balance FROM dbp_accounts WHERE company_id = :cid AND tenant_id = :t AND is_active = true ORDER BY code"
        ), {"cid": company_id, "t": tenant_id}).fetchall()
        result = []
        total_debit = 0
        total_credit = 0
        for a in accounts:
            ...
            total_debit += debit
            total_credit += credit
            result.append({"account_id": a[0], "account_code": a[1], "account_name": a[2], "account_type": a[3], "debit": debit, "credit": credit})
    ...
    def _next_entry_number(self, tenant_id: str, company_id: str) -> str:
        ...
        row = self.db.execute(text(
            "INSERT INTO number_sequences (id, tenant_id, name, prefix, current_number, increment_by, padding, entity_type, is_active) "
            "VALUES (:id, :t, :name, 'JE', 1, 1, 6, 'journal_entry', true) "
            "ON CONFLICT (tenant_id, name) DO UPDATE SET current_number = number_sequences.current_number + number_sequences.increment_by "
