...
def _redact_values(values: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    def _walk(obj):
        if isinstance(obj, dict):
            return {k: REDACT_VALUE if REDACT_KEY_PATTERNS.match(str(k)) else _walk(v) for k, v in obj.items()}
...
def log_dynamic_audit(
    ...
    """Write a redacted, correlation-aware audit entry without raising."""
    try:
        ...
        # Audit failures must not expose secrets or break the business transaction.
        safe_action = str(action).replace("\n", " ")[:100]
        safe_entity = str(entity_code).replace("\n", " ")[:100]
        print(f"[AUDIT-FAIL] {safe_action} on {safe_entity}: {type(e).__name__}", flush=True)
        return False
