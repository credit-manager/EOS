...
class PaymentGatewayEngine:
    ...
    def list_gateways(self, tenant_id):
        rows = self.db.execute(
            text("SELECT id, tenant_id, gateway_name, gateway_type, is_active, created_at FROM dbp_payment_gateways WHERE tenant_id=:t ORDER BY created_at DESC"),
            {"t": tenant_id},
        ).fetchall()
    ...
    def create_gateway(self, tenant_id, name, gw_type, config=None):
        ...
        gid = str(uuid.uuid4())
        self.db.execute(
            text("INSERT INTO dbp_payment_gateways (id, tenant_id, gateway_name, gateway_type, config) VALUES (:id, :t, :name, :type, CAST(:config AS JSONB))"),
            {"id": gid, "t": tenant_id, "name": gateway_name, "type": gateway_type, "config": self._serialize_gateway_config(config)},
        )
        self.db.commit()
        return {"gateway_id": gid, "message": f"Gateway {gateway_name} created"}

    def create_transaction(self, tenant_id, amount, currency="SAR", tx_type="payment", ref_type=None, ref_id=None, customer_id=None, method=None, idempotency_key: Optional[str] = None):
        ...
        if key:
            ...
            fingerprint = self._request_fingerprint(amount_value, normalized_currency, transaction_type, ref_type, ref_id, customer_id, method)
            existing = self.db.execute(
                text("SELECT id, status, idempotency_fingerprint FROM dbp_payment_transactions WHERE tenant_id=:t AND idempotency_key_hash=:kh FOR UPDATE"),
        ...
        try:
            self.db.execute(
                text("INSERT INTO dbp_payment_transactions (id, tenant_id, transaction_type, amount, currency, status, reference_type, reference_id, customer_id, payment_method, gateway_response, idempotency_key_hash, idempotency_fingerprint) VALUES (:id, :t, :type, :amt, :cur, 'pending', :rtype, :rid, :cid, :method, :resp, :kh, :fp)"),
                {"id": tid, "t": tenant_id, "type": transaction_type, "amt": amount_value, "cur": normalized_currency, "rtype": ref_type, "rid": ref_id, "cid": customer_id, "method": method, "resp": json.dumps({"ref_number": ref_number}), "kh": key_hash, "fp": fingerprint},
            )
            self.db.commit()
            ...
            if not key_hash:
                raise
            existing = self.db.execute(
                text("SELECT id, status, idempotency_fingerprint FROM dbp_payment_transactions WHERE tenant_id=:t AND idempotency_key_hash=:kh"),
                {"t": tenant_id, "kh": key_hash},
            ).fetchone()
            if not existing:
                raise exc
            if existing[2] != fingerprint:
                raise ValueError("Idempotency-Key was already used with different payment parameters") from exc
            return {"transaction_id": existing[0], "ref_number": None, "status": existing[1], "idempotent": True}
    ...
    def complete_transaction(self, transaction_id, tenant_id, gateway_response=None):
        ...
        self.db.execute(text("UPDATE dbp_payment_transactions SET status='completed', completed_at=NOW(), gateway_response = COALESCE(gateway_response,'{}'::jsonb) || :resp WHERE id=:id AND tenant_id=:t AND status='pending'"), {"id": transaction_id, "t": tenant_id, "resp": json.dumps(gateway_response or {})})
        self.db.commit()
    ...
    def fail_transaction(self, transaction_id, tenant_id, reason=""):
        ...
        self.db.execute(text("UPDATE dbp_payment_transactions SET status='failed', gateway_response = COALESCE(gateway_response,'{}'::jsonb) || :resp WHERE id=:id AND tenant_id=:t AND status='pending'"), {"id": transaction_id, "t": tenant_id, "resp": json.dumps({"failure_reason": str(reason)[:500]})})
        self.db.commit()
    ...
    def refund_transaction(self, transaction_id, tenant_id, amount=None):
        ...
        already_refunded = self.db.execute(text("SELECT COALESCE(SUM(amount),0) FROM dbp_payment_transactions WHERE reference_type='payment' AND reference_id=:ref AND transaction_type='refund' AND tenant_id=:t AND status='completed'"), {"ref": transaction_id, "t": tenant_id}).fetchone()[0]
        ...
        self.db.execute(text("INSERT INTO dbp_payment_transactions (id, tenant_id, transaction_type, amount, currency, status, reference_type, reference_id, completed_at) VALUES (:id,:t,'refund',:amt,:cur,'completed','payment',:ref,NOW())"), {"id": refund_id, "t": tenant_id, "amt": refund_amount, "cur": row_dict["currency"], "ref": transaction_id})
        self.db.commit()
    ...
    def create_payment_link(self, tenant_id, amount, description=None, email=None, expires_hours=24):
        ...
        self.db.execute(text("INSERT INTO dbp_payment_links (id, tenant_id, link_token, amount, currency, description, customer_email, expires_at) VALUES (:id,:t,:token,:amt,'SAR',:desc,:email,NOW() + (:hrs || ' hours')::interval)"), {"id": link_id, "t": tenant_id, "token": token, "amt": amount_value, "desc": description, "email": email, "hrs": str(hours)})
        self.db.commit()
    ...
    def process_bank_transfer(self, tenant_id, amount, bank_name, account_number, reference):
        ...
        self.db.execute(text("UPDATE dbp_payment_transactions SET gateway_response = COALESCE(gateway_response,'{}'::jsonb) || :resp WHERE id=:id AND tenant_id=:t"), {"id": tx["transaction_id"], "t": tenant_id, "resp": json.dumps({"bank_name": str(bank_name)[:120], "account_last4": str(account_number)[-4:], "transfer_reference": str(reference)[:120]})})
        self.db.commit()
        return tx
