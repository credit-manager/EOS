"""EOS Payment Gateway Engine with tenant isolation and idempotent writes."""
import hashlib
import json
import os
import secrets
...
class PaymentGatewayEngine:
    ...
    def create_transaction(self, tenant_id, amount, currency="SAR", tx_type="payment", ref_type=None, ref_id=None, customer_id=None, method=None, idempotency_key=None, gateway_metadata=None):
        if amount is None or float(amount) <= 0: raise ValueError("Payment amount must be positive")
        currency = (currency or "SAR").upper()
        if len(currency) != 3 or not currency.isalpha(): raise ValueError("Currency must be a 3-letter ISO code")
        ...
        if idempotency_key:
            ...
            if replay is not None: return replay.get("response_body") or {}
        tid = str(uuid.uuid4()); ref_number = f"TXN-{secrets.token_hex(4).upper()}"
        response = {"transaction_id": tid, "ref_number": ref_number, "status": "pending"}
        gateway_response = {"ref_number": ref_number, "metadata": metadata}
        try:
            ...
            if idempotency_key: idem.complete(tenant_id,idempotency_key,200,response)
            self.db.commit(); return response
        except Exception:
            self.db.rollback(); raise

    def complete_transaction(self, transaction_id, tenant_id, gateway_response=None, idempotency_key=None):
        payload={"transaction_id":transaction_id,"gateway_response":gateway_response or {},"operation":"complete"}; idem=IdempotencyStore(self.db)
        if idempotency_key:
            replay=idem.reserve(tenant_id,idempotency_key,payload)
            if replay is not None: return replay.get("response_body") or {}
        try:
            ...
            if not row: raise LookupError("Transaction not found")
            if row.status=="completed": response={"status":"completed","transaction_id":transaction_id}
            elif row.status=="failed": raise ValueError("Failed transaction cannot be completed")
            else:
    ...
    def fail_transaction(self, transaction_id, tenant_id, reason="", idempotency_key=None):
        payload={"transaction_id":transaction_id,"reason":reason,"operation":"fail"}; idem=IdempotencyStore(self.db)
        if idempotency_key:
            replay=idem.reserve(tenant_id,idempotency_key,payload)
            if replay is not None: return replay.get("response_body") or {}
        try:
            row=self.db.execute(text("SELECT id,status FROM dbp_payment_transactions WHERE id=:id AND tenant_id=:t FOR UPDATE"),{"id":transaction_id,"t":tenant_id}).fetchone()
            if not row: raise LookupError("Transaction not found")
            if row.status=="completed": raise ValueError("Completed transaction cannot be failed")
    ...
    def refund_transaction(self, transaction_id, tenant_id, amount=None, idempotency_key=None):
        idem=IdempotencyStore(self.db); payload={"transaction_id":transaction_id,"amount":str(amount) if amount is not None else None,"operation":"refund"}
        if idempotency_key:
            replay=idem.reserve(tenant_id,idempotency_key,payload)
            if replay is not None: return replay.get("response_body") or {}
        try:
            ...
            if not row: return {"error":"Transaction not found"}
            row_dict=dict(row._mapping)
            if row_dict["transaction_type"]=="refund": return {"error":"Cannot refund a refund transaction"}
            already=self.db.execute(text("SELECT COALESCE(SUM(amount),0) FROM dbp_payment_transactions WHERE reference_type='payment' AND reference_id=:ref AND transaction_type='refund' AND tenant_id=:t"),{"ref":transaction_id,"t":tenant_id}).fetchone()[0]
            refundable=Decimal(str(row_dict["amount"] or 0))-Decimal(str(already or 0)); refund_amount=refundable if amount is None else Decimal(str(amount))
            if refund_amount<=0: return {"error":"Nothing left to refund"}
            if refund_amount>refundable: return {"error":f"Refund amount {refund_amount} exceeds refundable {refundable} for this transaction"}
            refund_id=str(uuid.uuid4()); response={"refund_id":refund_id,"amount":float(refund_amount),"status":"completed"}
            self.db.execute(text("INSERT INTO dbp_payment_transactions (id,tenant_id,transaction_type,amount,currency,status,reference_type,reference_id) VALUES (:id,:t,'refund',:amt,:cur,'completed','payment',:ref)"),{"id":refund_id,"t":tenant_id,"amt":refund_amount,"cur":row_dict["currency"],"ref":transaction_id})
    ...
    def list_transactions(self, tenant_id, status=None, limit=50):
        query="SELECT * FROM dbp_payment_transactions WHERE tenant_id=:t"; params={"t":tenant_id}
        if status: query += " AND status=:s"; params["s"]=status
        query += " ORDER BY created_at DESC LIMIT :limit"; params["limit"]=limit
        return [dict(r._mapping) for r in self.db.execute(text(query),params).fetchall()]

    def get_transaction(self, transaction_id, tenant_id):
        row=self.db.execute(text("SELECT * FROM dbp_payment_transactions WHERE id=:id AND tenant_id=:t"),{"id":transaction_id,"t":tenant_id}).fetchone(); return dict(row._mapping) if row else None

    def create_payment_link(self, tenant_id, amount, description=None, email=None, expires_hours=24, idempotency_key=None):
        if amount is None or float(amount)<=0: raise ValueError("Payment amount must be positive")
        idem=IdempotencyStore(self.db); payload={"amount":str(amount),"description":description,"customer_email":email,"expires_hours":expires_hours,"operation":"create_payment_link"}
        if idempotency_key:
            replay=idem.reserve(tenant_id,idempotency_key,payload)
            if replay is not None: return replay.get("response_body") or {}
        link_id=str(uuid.uuid4()); token=secrets.token_urlsafe(32); response={"link_id":link_id,"payment_url":f"/pay/{token}","token":token}
        try:
    ...
    def process_bank_transfer(self, tenant_id, amount, bank_name, account_number, reference, idempotency_key=None):
        if not account_number: raise ValueError("Bank account number is required")
        if not bank_name: raise ValueError("Bank name is required")
        if not reference: raise ValueError("Bank transfer reference is required")
