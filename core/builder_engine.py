...
RESERVED_FIELD_CODES = {"id", "tenant_id", "created_at"}
FIELD_SQL_TYPES = {
    "string": "VARCHAR(255)",
    "text": "TEXT",
    "integer": "INTEGER",
...
class BuilderEngine:
    ...
    def create_project(self, tenant_id: str, name: str, composer_session_id: Optional[str] = None, initial_config: Optional[Dict] = None) -> Dict[str, Any]:
        pid = str(uuid.uuid4())
        draft = _new_draft_from_composer(initial_config)
        if composer_session_id:
            row = self.db.execute(text("SELECT generated_config FROM dbp_composer_sessions WHERE id=:sid AND tenant_id=:tid"), {"sid": composer_session_id, "tid": tenant_id}).fetchone()
            if not row:
        ...
        self.db.execute(text("INSERT INTO dbp_builder_projects (id, tenant_id, name, source_composer_session_id, status, draft_config) VALUES (:id,:tid,:name,:sid,'draft',:cfg)"), {"id": pid, "tid": tenant_id, "name": str(name).strip(), "sid": composer_session_id, "cfg": json.dumps(draft)})
        self.db.flush()
    ...
    def get_project(self, tenant_id: str, project_id: str) -> Optional[Dict[str, Any]]:
        row = self.db.execute(text("SELECT id,tenant_id,name,source_composer_session_id,status,draft_config,published_version_id,created_at,updated_at FROM dbp_builder_projects WHERE id=:pid AND tenant_id=:tid"), {"pid": project_id, "tid": tenant_id}).fetchone()
        if not row:
            return None
        cfg = row[5] if isinstance(row[5], dict) else json.loads(row[5])
        return {"id": row[0], "tenant_id": row[1], "name": row[2], "source_composer_session_id": row[3], "status": row[4], "draft_config": cfg, "published_version_id": row[6], "created_at": str(row[7]) if row[7] else None, "updated_at": str(row[8]) if row[8] else None}

    def list_projects(self, tenant_id: str) -> List[Dict[str, Any]]:
        rows = self.db.execute(text("SELECT id,name,status,created_at,updated_at FROM dbp_builder_projects WHERE tenant_id=:tid ORDER BY created_at DESC LIMIT 200"), {"tid": tenant_id}).fetchall()
        return [{"id": r[0], "name": r[1], "status": r[2], "created_at": str(r[3]) if r[3] else None, "updated_at": str(r[4]) if r[4] else None} for r in rows]

    def update_settings(self, tenant_id: str, pid: str, settings: Dict) -> bool:
        proj = self.get_project(tenant_id, pid)
        if not proj:
            return False
        merged = dict(proj["draft_config"].get("settings", {})); merged.update(settings or {})
    ...
    def set_modules(self, tenant_id: str, pid: str, modules: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        existing = {item["code"]: bool(item.get("enabled", True)) for item in proj["draft_config"].get("modules", []) if isinstance(item, dict) and item.get("code")}
        existing.update(seen)
        proj["draft_config"]["modules"] = [{"code": code, "enabled": enabled} for code, enabled in sorted(existing.items())]
    ...
    def add_entity(self, tenant_id: str, pid: str, entity_def: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in fields:
            ...
            if field_code in RESERVED_FIELD_CODES:
                return {"success": False, "error": f"Reserved field code: {field_code}"}
        ...
        entities[entity_code] = {"entity_code": entity_code, "name_en": entity_def["name_en"], "name_ar": entity_def.get("name_ar"), "faculty": entity_def.get("faculty", "operations"), "fields": fields}
    ...
    def remove_entity(self, tenant_id: str, pid: str, ecode: str) -> Dict[str, Any]:
        ...
        cfg["custom_entities"] = [entity for entity in cfg.get("custom_entities", []) if entity.get("entity_code") != ecode]
    ...
    def validate_draft(self, cfg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for entity in entities:
            ...
            for field in fields:
                ...
                if not isinstance(field_code, str) or not FIELD_CODE_RE.fullmatch(field_code):
                    ...
                    continue
                if field_code in RESERVED_FIELD_CODES:
                    errors.append(f"Reserved field code in '{code}': {field_code}")
        ...
        return {"valid": not errors, "errors": errors, "warnings": warnings, "summary": {"enabled_modules": len(enabled), "disabled_modules": sum(1 for item in modules if isinstance(item, dict) and not item.get("enabled")), "custom_entities": len(entities), "total_fields": sum(len(entity.get("fields", [])) for entity in entities if isinstance(entity, dict)), "relationships": len(cfg.get("relationships", [])) if isinstance(cfg, dict) else 0, "roles": len(cfg.get("roles", {})) if isinstance(cfg, dict) and isinstance(cfg.get("roles", {}), dict) else 0, "workflows": len(cfg.get("workflows", [])) if isinstance(cfg, dict) else 0, "kpis": len(cfg.get("kpis", [])) if isinstance(cfg, dict) else 0}}
    ...
    def publish(self, tenant_id: str, pid: str, published_by: str, confirmed: bool, change_summary: str = "") -> Dict[str, Any]:
        ...
        try:
            locked = self.db.execute(text("SELECT id FROM dbp_builder_projects WHERE id=:pid AND tenant_id=:tid FOR UPDATE"), {"pid": pid, "tid": tenant_id}).fetchone()
            if not locked:
                return {"success": False, "error": "Project not found"}
            created: List[str] = []
            registered: List[str] = []
            ...
            self.db.execute(text("INSERT INTO dbp_builder_versions (id,tenant_id,project_id,version_number,config,change_summary,published_by,is_active) VALUES (:id,:tid,:pid,:vn,:cfg,:sum,:by,true)"), {"id": version_id, "tid": tenant_id, "pid": pid, "vn": next_version, "cfg": json.dumps(cfg), "sum": change_summary or f"Version {next_version}", "by": published_by})
            self.db.execute(text("UPDATE dbp_builder_projects SET status='published',published_version_id=:vid,updated_at=NOW() WHERE id=:pid AND tenant_id=:tid"), {"vid": version_id, "pid": pid, "tid": tenant_id})
            self.db.commit()
            return {"success": True, "version_number": next_version, "version_id": version_id, "entities_published": registered, "tables_created": created, "validation": validation}
        except Exception:
            self.db.rollback()
    ...
    def list_versions(self, tenant_id: str, pid: str) -> List[Dict[str, Any]]:
        rows = self.db.execute(text("SELECT id,version_number,change_summary,published_by,published_at,is_active FROM dbp_builder_versions WHERE project_id=:pid AND tenant_id=:tid ORDER BY version_number DESC LIMIT 200"), {"pid": pid, "tid": tenant_id}).fetchall()
        return [{"id": row[0], "version_number": row[1], "change_summary": row[2], "published_by": row[3], "published_at": str(row[4]) if row[4] else None, "is_active": row[5]} for row in rows]
    ...
    def rollback(self, tenant_id: str, pid: str, version_id: str, rolled_back_by: str) -> Dict[str, Any]:
        ...
        try:
            ...
            current_codes = {entity.get("entity_code") for entity in proj["draft_config"].get("custom_entities", []) if entity.get("entity_code")}
            target_codes = {entity.get("entity_code") for entity in target_cfg.get("custom_entities", []) if entity.get("entity_code")}
            removed: List[str] = []
            restored: List[str] = []
            ...
            self.db.execute(text("UPDATE dbp_builder_versions SET is_active=false WHERE project_id=:pid AND tenant_id=:tid"), {"pid": pid, "tid": tenant_id})
            self.db.execute(text("INSERT INTO dbp_builder_versions (id,tenant_id,project_id,version_number,config,change_summary,published_by,is_active) VALUES (:id,:tid,:pid,:vn,:cfg,:sum,:by,true)"), {"id": new_version_id, "tid": tenant_id, "pid": pid, "vn": next_version, "cfg": json.dumps(target_cfg), "sum": f"Rollback to version snapshot {version_id}", "by": rolled_back_by})
            self.db.execute(text("UPDATE dbp_builder_projects SET status='published',draft_config=:cfg,published_version_id=:vid,updated_at=NOW() WHERE id=:pid AND tenant_id=:tid"), {"cfg": json.dumps(target_cfg), "vid": new_version_id, "pid": pid, "tid": tenant_id})
            self.db.commit()
            return {"success": True, "rolled_back_to_version_id": version_id, "new_version_number": next_version, "entities_restored": restored, "entities_removed": removed}
        except Exception:
            self.db.rollback()
    ...
    def _ensure_physical_table(self, table_name: str, fields: List[Dict[str, Any]]) -> None:
        ...
        for field in fields:
            ...
            if code in RESERVED_FIELD_CODES:
                raise ValueError(f"Reserved field code: {code}")
            sql_type = FIELD_SQL_TYPES.get(field_type)
            if not sql_type:
                raise ValueError(f"Unsupported field type: {field_type}")
            columns.append({"code": code, "sql_type": sql_type, "not_null": bool(field.get("is_required"))})
        self.db.execute(text("SELECT public.eos_create_builder_table(:table_name, CAST(:columns AS JSONB))"), {"table_name": table_name, "columns": json.dumps(columns)})

    def _register_entity(self, tenant_id: str, table_name: str, entity: Dict[str, Any]) -> None:
        existing = self.db.execute(text("SELECT id,tenant_id,is_system FROM dbp_entities WHERE code=:code AND tenant_id=:tid"), {"code": entity["entity_code"], "tid": tenant_id}).fetchone()
        if existing:
            if existing[2]:
        ...
        self.db.execute(text("INSERT INTO dbp_entities (id,tenant_id,code,name_en,name_ar,faculty,table_mapping,is_system,metadata_schema) VALUES (:id,:tid,:code,:nen,:nar,:fac,:tbl,false,'{}')"), {"id": entity_id, "tid": tenant_id, "code": entity["entity_code"], "nen": entity["name_en"], "nar": entity.get("name_ar"), "fac": entity.get("faculty", "operations"), "tbl": table_name})
    ...
    def _sync_fields(self, entity_id: str, entity: Dict[str, Any]) -> None:
        ...
        for order, (code, field) in enumerate(desired.items(), 1):
            enums = json.dumps(field.get("enum_values", [])); ui = json.dumps({"component": "input", "order": order})
            params = {"label_en": field.get("label_en", code), "label_ar": field.get("label_ar"), "field_type": field["field_type"], "required": bool(field.get("is_required")), "enums": enums, "ui": ui, "eid": entity_id, "code": code}
            if code in current_map:
                self.db.execute(text("UPDATE dbp_fields SET label_en=:label_en,label_ar=:label_ar,field_type=:field_type,is_required=:required,enum_values=CAST(:enums AS JSONB),ui_config=CAST(:ui AS JSONB) WHERE entity_id=:eid AND code=:code"), params)
            else:
                self.db.execute(text("INSERT INTO dbp_fields (id,entity_id,code,label_en,label_ar,field_type,is_required,ui_config,enum_values) VALUES (:id,:eid,:code,:label_en,:label_ar,:field_type,:required,CAST(:ui AS JSONB),CAST(:enums AS JSONB))"), {"id": str(uuid.uuid4()), **params})
        self.db.flush()

    def _unregister_entity(self, tenant_id: str, table_name: str, entity_code: str) -> None:
        row = self.db.execute(text("SELECT id FROM dbp_entities WHERE code=:code AND tenant_id=:tid AND is_system=false"), {"code": entity_code, "tid": tenant_id}).fetchone()
        if not row:
            return
