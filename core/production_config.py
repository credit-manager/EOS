...
def validate_production_config() -> List[Tuple[str, str, bool]]:
    ...
    check("DATABASE_URL", os.getenv("DATABASE_URL", ""), r"^postgresql://.{10,}$", critical=True)
    ...
    if payment_mode == "stripe":
        ...
        check("EOS_STRIPE_SECRET_KEY", stripe_key, r"^sk_live_.{10,}$", critical=True)
    ...
    check("EOS_TRUSTED_HOSTS_ENABLED", os.getenv("EOS_TRUSTED_HOSTS_ENABLED", ""), r"^true$", critical=True)
    check("EOS_DISABLE_DOCS", os.getenv("EOS_DISABLE_DOCS", ""), r"^true$", critical=True)
    check("EOS_RATE_LIMIT_FAIL_CLOSED", os.getenv("EOS_RATE_LIMIT_FAIL_CLOSED", ""), r"^true$", critical=True)
    ...
    check("DOMAIN", os.getenv("DOMAIN", ""), r"^https://", critical=True)
