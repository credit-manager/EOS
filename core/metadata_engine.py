...
class MetadataEngine:
    ...
    def get_entity_by_code(self, code: str, tenant_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        if tenant_id is not None:
            query = query.filter((DBPEntity.tenant_id == tenant_id) | (DBPEntity.tenant_id.is_(None)))
        entity = query.first()
        if not entity:
            return None
        return {k: v for k, v in entity.__dict__.items() if k != "_sa_instance_state"}

    def get_entity_fields(self, entity_id: str, tenant_id: Optional[str] = None) -> list:
        ...
        if tenant_id is not None:
            query = query.filter((DBPEntity.tenant_id == tenant_id) | (DBPEntity.tenant_id.is_(None)))
        fields = query.order_by(text("CAST(ui_config->>'order' AS INTEGER) ASC")).all()
        return [{k: v for k, v in field.__dict__.items() if k != "_sa_instance_state"} for field in fields]

    def get_full_schema(self, code: str, tenant_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {"entity": entity, "fields": self.get_entity_fields(entity["id"], tenant_id=tenant_id)}

    def validate_data(self, code: str, data: Dict[str, Any], tenant_id: Optional[str] = None):
