...
class ERPFoundationEngine:
    ...
    def update_company(self, company_id: str, data: Dict[str, Any]) -> bool:
        ...
        if not sets:
            return False
        self.db.execute(text(f"UPDATE dbp_companies SET {', '.join(sets)} WHERE id = :cid"), params)
        self.db.flush()
        return True
    ...
    def get_branches(self, company_id: str) -> List[Dict]:
        rows = self.db.execute(text(
            "SELECT id, code, name_en, name_ar, city, is_headquarters, is_active "
            "FROM dbp_branches WHERE company_id = :cid ORDER BY code"
    ...
    def get_fiscal_years(self, company_id: str) -> List[Dict]:
        rows = self.db.execute(text(
            "SELECT id, code, name, start_date, end_date, is_closed, is_active "
            "FROM dbp_fiscal_years WHERE company_id = :cid ORDER BY start_date DESC"
        ), {"cid": company_id}).fetchall()
    ...
    def close_fiscal_year(self, fy_id: str) -> bool:
        result = self.db.execute(text(
            "UPDATE dbp_fiscal_years SET is_closed = true WHERE id = :fid AND is_closed = false"
        ), {"fid": fy_id})
        self.db.flush()
    ...
    def get_cost_centers(self, company_id: str) -> List[Dict]:
        rows = self.db.execute(text(
            "SELECT id, parent_id, code, name_en, name_ar, budget_amount, is_active "
            "FROM dbp_cost_centers WHERE company_id = :cid ORDER BY code"
        ), {"cid": company_id}).fetchall()
