...
def _get_production_auth():
    ...
    from core.production_auth import security as prod_security
    from core.production_auth import verify_token as prod_verify_token
    return prod_security, prod_verify_token


def _get_test_auth():
    """Lazy import of test auth module."""
    from core.auth import security as test_security
    from core.auth import verify_token as test_verify_token
...
async def get_current_user(
    ...
    if tenant_id is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Token missing tenant ID"
        )
