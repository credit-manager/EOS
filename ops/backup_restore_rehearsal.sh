...
psql "${RESTORE_DATABASE_URL}" -v ON_ERROR_STOP=1 <<'SQL'
SELECT 'eos_v2_accounts' AS table_name, COUNT(*) AS rows FROM eos_v2_accounts;
SELECT 'eos_v2_journal_entries' AS table_name, COUNT(*) AS rows FROM eos_v2_journal_entries;
SELECT 'eos_v2_journal_lines' AS table_name, COUNT(*) AS rows FROM eos_v2_journal_lines;
SELECT COUNT(*) AS orphan_lines
FROM eos_v2_journal_lines l
LEFT JOIN eos_v2_journal_entries e
