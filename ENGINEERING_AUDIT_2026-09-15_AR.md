...
**تحديث التنفيذ في الفرع الحالي:** entrypoint المتصفح يمر الآن إلى runtime واحد هو `PlatformApp`. يستخدم كتالوج/كيانات/استديو metadata، وAPI client موحد، وhash routes للـ workspace/objects/builder، وanalytics home feed، وrefresh rotation وserver logout. ما زالت واجهات workflow/rules/events/graph/AI ورحلة demo الكاملة غير مكتملة.
