...
async def list_accounts(
    ...
    count_row = db.execute(text(f"SELECT COUNT(*) FROM dbp_accounts WHERE {where}"), params).fetchone()
    ...
    rows = db.execute(
        text(f"SELECT id, code, name_en, name_ar, account_type, parent_id, currency_code, "
    ...
    data = [{"id": r[0], "code": r[1], "name": r[2], "name_ar": r[3],
             ...
             "current_balance": float(r[10]) if r[10] else 0,
             "description": r[11]}
            for r in rows]
...
async def create_account(body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.execute(
        text("INSERT INTO dbp_accounts (id, tenant_id, company_id, code, name_en, name_ar, "
             "account_type, parent_id, currency_code, is_active, is_system, opening_balance, "
...
async def list_journal_entries(
    ...
    if end_date:
        conditions.append("entry_date <= :ed")
        params["ed"] = end_date
    if status:
    ...
    count_row = db.execute(text(f"SELECT COUNT(*) FROM dbp_journal_entries WHERE {where}"), params).fetchone()
    total = count_row[0] if count_row else 0
    offset = (page - 1) * page_size
    params["limit"] = page_size
    params["offset"] = offset
    ...
    rows = db.execute(
        text(f"SELECT id, entry_number, entry_date, entry_type, description, reference, "
             f"status, total_debit, total_credit, is_posted, created_by "
             f"FROM dbp_journal_entries WHERE {where} ORDER BY entry_date DESC LIMIT :limit OFFSET :offset"),
        params,
    ).fetchall()

    data = [{"id": r[0], "entry_number": r[1],
             "entry_date": r[2].isoformat() if r[2] else None,
             "entry_type": r[3], "description": r[4], "reference": r[5],
             "status": r[6] or "draft",
             "total_debit": float(r[7]) if r[7] else 0,
             "total_credit": float(r[8]) if r[8] else 0,
             "is_posted": r[9] or False, "created_by": r[10]}
            for r in rows]
    return {"data": data, "total": total, "page": page, "page_size": page_size}
...
async def get_journal_entry(entry_id: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    lines = db.execute(
        text("SELECT jl.id, jl.account_id, a.code, a.name_en, jl.debit, jl.credit, "
             "jl.description, jl.cost_center_id "
             "FROM dbp_journal_lines jl LEFT JOIN dbp_accounts a ON jl.account_id = a.id "
             "WHERE jl.journal_entry_id = :eid ORDER BY jl.line_order"),
        {"eid": entry_id},
    ).fetchall()
    ...
    return {
        ...
        "lines": [{"id": l[0], "account_id": l[1], "account_code": l[2],
...
async def create_journal_entry(body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for i, line in enumerate(lines):
        lid = str(uuid.uuid4())
        db.execute(
            text("INSERT INTO dbp_journal_lines (id, journal_entry_id, account_id, debit, credit, "
                 "currency_code, description, cost_center_id, line_order, created_at) "
             "VALUES (:id, :eid, :aid, :dr, :cr, :cur, :desc, :cc, :lo, :now)"),
            {"id": lid, "eid": eid, "aid": line.get("account_id"),
             "dr": line.get("debit", 0), "cr": line.get("credit", 0),
...
async def trial_balance(
    ...
    total_debit = sum(a["balance"] for a in accounts if a["balance"] > 0 and a["account_type"] in ("asset", "expense"))
    total_credit = sum(abs(a["balance"]) for a in accounts if a["balance"] < 0 and a["account_type"] in ("liability", "equity", "revenue"))
    return {"accounts": accounts, "total_debit": total_debit, "total_credit": total_credit}
...
async def balance_sheet(
    ...
    equity = sum(abs(float(r[3])) for r in rows if r[2] == "equity" and r[3])
...
async def profit_and_loss(
    ...
    revenue = sum(float(r[3]) for r in rows if r[2] == "revenue" and r[3])
    expenses = sum(abs(float(r[3])) for r in rows if r[2] == "expense" and r[3])
    return {"revenue": revenue, "cogs": 0, "gross_profit": revenue,
            "operating_expenses": expenses, "net_income": revenue - expenses}
