...
async def list_customers(cid: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    return {"status": "success", "data": SalesEngine(db).list_customers(cid, tenant_id=user["tenant_id"])}
...
async def create_customer(cid: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    cust_id = SalesEngine(db).create_customer(user["tenant_id"], cid, body["name"], contact_name=body.get("contact_name"), email=body.get("email"), phone=body.get("phone"), address=body.get("address"), payment_terms=body.get("payment_terms"), credit_limit=body.get("credit_limit", 0))
    db.commit()
...
async def list_quotations(cid: str, status: str | None = None, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    conditions = ["company_id = :cid", "tenant_id = :tid"]
    params = {"cid": cid, "tid": user["tenant_id"]}
    if status:
...
async def create_quotation(cid: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    qid = SalesEngine(db).create_quotation(user["tenant_id"], cid, body["customer_id"], body["quote_date"], body["lines"], created_by=user.get("id") or "system", valid_until=body.get("valid_until"), notes=body.get("notes"))
    db.commit()
...
async def convert_quotation(qid: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    q = db.execute(sa("SELECT company_id FROM dbp_sales_quotations WHERE id = :qid AND tenant_id = :tid"), {"qid": qid, "tid": user["tenant_id"]}).fetchone()
    if not q:
        raise HTTPException(404, detail={"status": "error", "error": {"code": "NOT_FOUND", "message": "Quotation not found"}})
    result = SalesEngine(db).convert_quotation_to_order(qid, user["tenant_id"], q[0], created_by=user.get("id") or "admin")
...
async def list_sales_orders(cid: str, status: str | None = None, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    return {"status": "success", "data": SalesEngine(db).list_sales_orders(cid, status=status, tenant_id=user["tenant_id"])}
...
async def create_sales_order(cid: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    for f in ("customer_id", "order_date", "lines"):
        if f not in body:
            raise HTTPException(400, detail={"status": "error", "error": {"code": "MISSING", "message": f"{f} required"}})
    oid = SalesEngine(db).create_sales_order(user["tenant_id"], cid, body["customer_id"], body["order_date"], body["lines"], currency_code=body.get("currency_code"), notes=body.get("notes"), created_by=user.get("id") or "system")
    db.commit()
...
async def get_sales_order(oid: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    order = SalesEngine(db).get_sales_order(oid, user["tenant_id"])
    if not order:
...
async def list_invoices(cid: str, status: str | None = None, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    return {"status": "success", "data": SalesEngine(db).list_invoices(cid, status=status, tenant_id=user["tenant_id"])}
...
async def create_invoice(cid: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    for f in ("customer_id", "invoice_date", "lines"):
        if f not in body:
            raise HTTPException(400, detail={"status": "error", "error": {"code": "MISSING", "message": f"{f} required"}})
    iid = SalesEngine(db).create_invoice(user["tenant_id"], cid, body["customer_id"], body["invoice_date"], body["lines"], order_id=body.get("order_id"), due_date=body.get("due_date"), currency_code=body.get("currency_code"), notes=body.get("notes"), created_by=user.get("id") or "system")
    db.commit()
...
async def get_invoice(iid: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    inv = SalesEngine(db).get_invoice(iid, user["tenant_id"])
    if not inv:
...
async def record_payment(iid: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    result = SalesEngine(db).record_payment(iid, body["amount"], body.get("payment_date", ""), user["tenant_id"])
    if not result["success"]:
