...
async def convert_quotation(qid: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    q = db.execute(sa("SELECT company_id FROM dbp_sales_quotations WHERE id = :qid"), {"qid": qid}).fetchone()
    if not q:
...
async def get_sales_order(oid: str, db: Session = Depends(get_db)):
    order = SalesEngine(db).get_sales_order(oid)
    if not order:
...
async def get_invoice(iid: str, db: Session = Depends(get_db)):
    inv = SalesEngine(db).get_invoice(iid)
    if not inv:
...
async def record_payment(iid: str, body: dict, db: Session = Depends(get_db)):
