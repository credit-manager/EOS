...
async def list_customers(
    ...
    count_row = db.execute(text(f"SELECT COUNT(*) FROM customers WHERE {where}"), params).fetchone()
    ...
    rows = db.execute(
        text(f"SELECT id, name, name_ar, type, email, phone, address, tax_id, "
             f"total_orders, total_spent, created_at, is_active "
    ...
    for r in rows:
        data.append({
            ...
            "name": r[1],
            "name_ar": r[2],
            "type": r[3] or "individual",
            "email": r[4],
            "phone": r[5],
            "address": r[6],
            "tax_id": r[7],
            "total_orders": r[8] or 0,
            "total_spent": float(r[9]) if r[9] else 0,
...
async def get_customer(
    ...
    r = db.execute(
        text("SELECT id, name, name_ar, type, email, phone, address, tax_id, "
             "total_orders, total_spent, created_at, is_active "
...
async def create_customer(
    ...
    db.execute(
        text("INSERT INTO customers (id, name, name_ar, type, email, phone, address, tax_id, "
             "total_orders, total_spent, created_at, updated_at, is_active) "
             "VALUES (:id, :name, :name_ar, :type, :email, :phone, :address, :tax_id, "
             "0, 0, :now, :now, true)"),
        {
            "id": cid,
            "name": body["name"],
...
async def update_customer(
    ...
    fields = []
    params: dict = {"id": customer_id, "now": datetime.now(timezone.utc)}
    ...
    if fields:
        ...
        db.execute(text(f"UPDATE customers SET {', '.join(fields)} WHERE id = :id"), params)
        db.commit()
...
async def list_leads(
    ...
    try:
        ...
        count_row = db.execute(text(f"SELECT COUNT(*) FROM leads WHERE {where}"), params).fetchone()
        total = count_row[0] if count_row else 0
        ...
        params["offset"] = offset
        rows = db.execute(
            text(f"SELECT id, first_name, last_name, company_name, email, phone, source, status, score, notes, created_at "
        ...
        for r in rows:
            data.append({
                ...
                "company_name": r[3], "email": r[4], "phone": r[5],
                "source": r[6], "status": r[7] or "new", "score": r[8] or 0,
...
async def create_lead(
    ...
    try:
        db.execute(
            text("INSERT INTO leads (id, first_name, last_name, company_name, email, phone, source, status, score, notes, created_at) "
                 "VALUES (:id, :fn, :ln, :cn, :email, :phone, :src, 'new', 0, :notes, :now)"),
            {"id": lid, "fn": body.get("first_name", ""), "ln": body.get("last_name", ""),
        ...
        db.rollback()
        raise HTTPException(400, detail="Failed to create lead (table may not exist)")
...
async def list_opportunities(
    ...
    try:
        ...
        count_row = db.execute(text(f"SELECT COUNT(*) FROM opportunities WHERE {where}"), params).fetchone()
        total = count_row[0] if count_row else 0
        ...
        params["offset"] = offset
        rows = db.execute(
            text(f"SELECT id, name, customer_id, stage, amount, probability, expected_close_date, notes, created_at "
        ...
        for r in rows:
            data.append({
                "id": r[0], "name": r[1], "customer_id": r[2],
                "stage": r[3] or "prospecting", "amount": float(r[4]) if r[4] else 0,
...
async def create_opportunity(
    ...
    try:
        db.execute(
            text("INSERT INTO opportunities (id, name, customer_id, stage, amount, probability, notes, created_at) "
                 "VALUES (:id, :name, :cid, :stage, :amt, :prob, :notes, :now)"),
            {"id": oid, "name": body.get("name", ""), "cid": body.get("customer_id"),
        ...
        db.rollback()
        raise HTTPException(400, detail="Failed to create opportunity (table may not exist)")
