...
from core.auth import require_permission, get_current_user, require_admin_role
...
@router.post("/providers", dependencies=[Depends(require_admin_role), Depends(write_limiter.check)])
async def create_provider(body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(400, detail={"status": "error", "error": {"code": "VALIDATION_ERROR", "message": str(exc)}})
    return {"status": "success", "data": {"id": pid, "message": "Provider created"}}


@router.put("/providers/{provider_id}", dependencies=[Depends(require_admin_role), Depends(write_limiter.check)])
...
async def list_sessions(user_id: str = None, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if user_id and user_id != user.get("id") and not ({"admin", "platform_owner"} & {str(r) for r in user.get("roles", []) if isinstance(r, str)}):
        raise HTTPException(403, detail="Administrator privileges required")
    return {"status": "success", "data": IdentityEngine(db).list_sessions(user["tenant_id"], user_id=user_id or user.get("id"))}


@router.post("/sessions", dependencies=[Depends(require_admin_role), Depends(write_limiter.check)])
...
async def list_mfa(user_id: str = None, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    roles = {str(r) for r in user.get("roles", []) if isinstance(r, str)}
    if user_id and user_id != user.get("id") and not ({"admin", "platform_owner"} & roles):
        raise HTTPException(403, detail="Administrator privileges required")
    return {"status": "success", "data": IdentityEngine(db).list_mfa(user["tenant_id"], user_id=user_id or user.get("id"))}
...
async def setup_mfa(body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if target_user_id != user.get("id") and not ({"admin", "platform_owner"} & roles):
        raise HTTPException(403, detail="Administrator privileges required")
    result = IdentityEngine(db).setup_mfa(user["tenant_id"], target_user_id, body["mfa_type"])
    db.commit()
    return {"status": "success", "data": result}
...
@router.post("/role-mappings", dependencies=[Depends(require_admin_role), Depends(write_limiter.check)])
...
@router.delete("/role-mappings/{mapping_id}", dependencies=[Depends(require_admin_role), Depends(write_limiter.check)])
...
@router.post("/api-keys", dependencies=[Depends(require_admin_role), Depends(write_limiter.check)])
...
@router.put("/api-keys/{key_id}/revoke", dependencies=[Depends(require_admin_role), Depends(write_limiter.check)])
