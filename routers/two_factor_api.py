...
@router.post("/verify")
def verify(body: Verify2FA, request: Request, user: dict = Depends(get_current_user), db=Depends(get_db)):
...
def verify_recovery(body: VerifyRecovery, request: Request, user: dict = Depends(get_current_user), db=Depends(get_db)):
    ip = request.client.host if request.client else None
