...
async def create_entity(body: dict, db: Session=None, current_user: dict = Depends(get_current_user)):
    ...
    entity = DBPEntity(id=entity_id, tenant_id=tenant_id, code=code, name_en=body["name_en"],
                       name_ar=body.get("name_ar"), faculty=body["faculty"], table_mapping=table_mapping,
                       is_system=body.get("is_system", False) if is_platform_owner(current_user) else False,
    ...
    for fd in body.get("fields", []):
        fcode = fd.get("code")
        if not fcode:
            raise HTTPException(400, "Field missing 'code'")
        _validate_field_code(fcode)
        ftype = fd.get("field_type", "string")
        if ftype not in ALLOWED_FIELD_TYPES:
            raise HTTPException(400, detail=f"Invalid field_type: {ftype}")
        db.add(DBPField(id=str(uuid.uuid4()), entity_id=entity_id, code=fcode,
                        label_en=fd.get("label_en"), label_ar=fd.get("label_ar"), field_type=ftype,
                        is_required=fd.get("is_required", False), ui_config=fd.get("ui_config", {}),
...
async def update_entity(entity_code: str, body: dict, db: Session=None, current_user: dict = Depends(get_current_user)):
    ...
    for name in ("name_en", "name_ar", "faculty", "metadata_schema"):
        if name in body and body[name] != getattr(entity, name):
            setattr(entity, name, body[name]); changed_fields.append(name)
...
async def delete_entity(entity_code: str, db: Session=None, current_user: dict = Depends(get_current_user)):
    ...
    if not entity:
        raise HTTPException(404, "Entity not found")
    if entity.is_system:
        raise HTTPException(403, "Cannot delete system entity")
    rec_count = db.execute(text(f"SELECT COUNT(*) FROM {entity.table_mapping}")).scalar()
    if rec_count > 0:
        raise HTTPException(409, detail=f"Entity has {rec_count} records. Delete records first.")
...
async def add_field(entity_code: str, body: dict, db: Session=None, current_user: dict = Depends(get_current_user)):
    ...
    _validate_field_code(fcode)
    if db.query(DBPField).filter(DBPField.entity_id == entity.id, DBPField.code == fcode).first():
        raise HTTPException(409, detail=f"Field '{fcode}' already exists")
    ftype = body.get("field_type", "string")
    if ftype not in ALLOWED_FIELD_TYPES:
        raise HTTPException(400, detail=f"Invalid field_type: {ftype}")
    db.add(DBPField(id=str(uuid.uuid4()), entity_id=entity.id, code=fcode, label_en=body.get("label_en"),
                    label_ar=body.get("label_ar"), field_type=ftype, is_required=body.get("is_required", False),
                    ui_config=body.get("ui_config", {}), enum_values=body.get("enum_values", [])))
    db.flush()
...
async def update_field(entity_code: str, field_code: str, body: dict, db: Session=None, current_user: dict = Depends(get_current_user)):
    ...
    for name in ("label_en", "label_ar", "is_required", "ui_config", "enum_values"):
        if name in body and body[name] != getattr(field, name):
            setattr(field, name, body[name]); changed.append(name)
    if "field_type" in body:
        if body["field_type"] not in ALLOWED_FIELD_TYPES:
            raise HTTPException(400, detail=f"Invalid field_type: {body['field_type']}")
        if body["field_type"] != field.field_type:
            field.field_type = body["field_type"]; changed.append("field_type")
    if not changed:
        return {"status": "success", "message": "No changes"}
    db.flush()
