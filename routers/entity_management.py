...
async def create_entity(body: dict, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    for field in required:
        if field not in body: raise HTTPException(400, detail=f"Missing required field: {field}")
    code = body["code"]; _validate_code(code)
    tenant_id = _tenant_for_write(current_user, body.get("tenant_id"))
    existing_q = db.query(DBPEntity).filter(DBPEntity.code == code)
    existing_q = existing_q.filter(DBPEntity.tenant_id.is_(None) if tenant_id is None else DBPEntity.tenant_id == tenant_id)
    if existing_q.first(): raise HTTPException(409, detail=f"Entity '{code}' already exists for this tenant")
    table_mapping = body["table_mapping"]
    if not re.match(r'^[a-z][a-z0-9_]{0,99}$', table_mapping): raise HTTPException(400, "table_mapping must be safe")
    ...
    if not table_check: raise HTTPException(400, detail=f"Physical table '{table_mapping}' does not exist")
    ...
    for fd in body.get("fields", []):
        ...
        _validate_field_code(fcode); ftype = fd.get("field_type", "string")
    ...
    db.commit(); return {"status": "success", "entity_id": entity_id, "version": ver["version_number"]}
...
async def update_entity(entity_code: str, body: dict, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    db.flush(); ver = VersioningEngine(db).create_version(entity_id=entity.id, change_type="update_entity", changed_by=current_user.get("id", current_user.get("user_id", "unknown")), change_summary=f"Updated: {', '.join(changed_fields)}")
    EventBus(db).emit("entity.updated", entity_code, tenant_id=entity.tenant_id, user_id=current_user.get("id", current_user.get("user_id")), payload={"changed_fields": changed_fields}); db.commit()
...
async def delete_entity(entity_code: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    tenant_id = entity.tenant_id; db.delete(entity)
    EventBus(db).emit("entity.deleted", entity_code, tenant_id=tenant_id, user_id=current_user.get("id", current_user.get("user_id"))); db.commit()
...
async def add_field(entity_code: str, body: dict, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    if ftype not in ALLOWED_FIELD_TYPES: raise HTTPException(400, detail=f"Invalid field_type: {ftype}")
    db.add(DBPField(id=str(uuid.uuid4()), entity_id=entity.id, code=fcode, label_en=body.get("label_en"), label_ar=body.get("label_ar"), field_type=ftype, is_required=body.get("is_required", False), ui_config=body.get("ui_config", {}), enum_values=body.get("enum_values", []))); db.flush()
    ...
    EventBus(db).emit("field.added", entity_code, tenant_id=entity.tenant_id, user_id=current_user.get("id", current_user.get("user_id")), payload={"field_code": fcode, "field_type": ftype}); db.commit()
...
async def update_field(entity_code: str, field_code: str, body: dict, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    entity = _field_entity(db, entity_code, current_user); field = db.query(DBPField).filter(DBPField.entity_id == entity.id, DBPField.code == field_code).first()
    ...
    db.flush(); ver = VersioningEngine(db).create_version(entity_id=entity.id, change_type="update_field", changed_by=current_user.get("id", current_user.get("user_id", "unknown")), change_summary=f"Updated field '{field_code}': {', '.join(changed)}")
    EventBus(db).emit("field.updated", entity_code, tenant_id=entity.tenant_id, user_id=current_user.get("id", current_user.get("user_id")), payload={"field_code": field_code, "changed": changed}); db.commit()
...
async def remove_field(entity_code: str, field_code: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    if not field: raise HTTPException(404, "Field not found")
    db.delete(field); db.flush(); ver = VersioningEngine(db).create_version(entity_id=entity.id, change_type="remove_field", changed_by=current_user.get("id", current_user.get("user_id", "unknown")), change_summary=f"Removed field '{field_code}'")
    EventBus(db).emit("field.removed", entity_code, tenant_id=entity.tenant_id, user_id=current_user.get("id", current_user.get("user_id")), payload={"field_code": field_code}); db.commit()
...
async def list_versions(entity_code: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    entity = _field_entity(db, entity_code, current_user); versions = VersioningEngine(db).get_versions(entity.id)
...
async def get_version(entity_code: str, version_number: int, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    entity = _field_entity(db, entity_code, current_user); version = VersioningEngine(db).get_version(entity.id, version_number)
