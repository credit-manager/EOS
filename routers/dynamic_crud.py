...
# ──────────────────────────────────────────────────────────────
# SQL SAFETY — Identifier validation
# ──────────────────────────────────────────────────────────────
...
def _validate_identifier(name: str, label: str = "identifier") -> str:
    """
    Validate that a string is a safe SQL identifier (lowercase alphanumeric + underscore).
    Prevents SQL injection via table/column names from metadata.
    Raises HTTPException if invalid.
    """
    if not name or not _SAFE_IDENTIFIER.match(name):
        raise HTTPException(
            status_code=400,
            detail=f"Invalid {label}: '{name}'. Must be lowercase alphanumeric with underscores."
        )
    return name
...
async def list_records(
    ...
    _validate_identifier(table_mapping, "table_mapping")
    ...
    if include:
        ...
        if ent_row:
            ...
            for inc in include_list:
                ...
                # BATCH LOADING: Collect all source values first, then query once
                source_values = []
                for record in records:
                    sv = record.get(rel_data["source_column"])
                    if sv is not None:
                ...
                unique_values = list(set(source_values))[:500]
                ...
                if rel_data["relationship_type"] == "many_to_many":
                    ...
                    if not junction or not j_src or not j_tgt:
                        for record in records:
                            record[inc] = []
                        continue
                    _validate_identifier(junction, "junction_table")
                    _validate_identifier(j_src, "junction_source_col")
                    _validate_identifier(j_tgt, "junction_target_col")
                    _validate_identifier(rel_data["target_column"], "target_column")
                    _validate_identifier(target_table, "target_table")
                    placeholders = ", ".join([f":sv{i}" for i in range(len(unique_values))])
                    q = text(
                        f"SELECT t.*, j.{j_src} AS _src_val FROM {target_table} t "
                        f"INNER JOIN {junction} j "
                        f"ON j.{j_tgt} = t.{rel_data['target_column']} "
                        f"WHERE j.{j_src} IN ({placeholders}) LIMIT 1000"
                    )
                    q_params = {f"sv{i}": v for i, v in enumerate(unique_values)}
                    rows = db.execute(q, q_params).fetchall()
                    by_src: dict = {}
                    for row in rows:
                        src_val = str(row._mapping["_src_val"])
                        if src_val not in by_src:
                            by_src[src_val] = []
                        cols = [c for c in row._mapping.keys() if c not in ("tenant_id", "_src_val")]
                        by_src[src_val].append({col: row._mapping[col] for col in cols})
                    for record in records:
                        sv = record.get(rel_data["source_column"])
                        record[inc] = by_src.get(str(sv), []) if sv else []

                elif rel_data["relationship_type"] == "lookup":
                    lookup_col = rel_data["lookup_field"]
                    _validate_identifier(lookup_col, "lookup_field")
                    _validate_identifier(rel_data["target_column"], "target_column")
                    _validate_identifier(target_table, "target_table")
                    placeholders = ", ".join([f":sv{i}" for i in range(len(unique_values))])
                    where_parts = [f"{rel_data['target_column']} IN ({placeholders})"]
                    l_params: dict = {f"sv{i}": v for i, v in enumerate(unique_values)}
                    if rel_data["tenant_scope"] and effective_tenant:
                        where_parts.append("tenant_id = :tid")
                        l_params["tid"] = effective_tenant
                    where_sql = " AND ".join(where_parts)
                    q = text(
                        f"SELECT id, {lookup_col}, {rel_data['target_column']} FROM {target_table} "
                        f"WHERE {where_sql} "
                        f"ORDER BY {lookup_col} ASC LIMIT 1000"
                    )
                    rows = db.execute(q, l_params).fetchall()
                    by_src = {}
                    for r in rows:
                        src_val = str(r[2])
                        if src_val not in by_src:
                            by_src[src_val] = []
                        by_src[src_val].append({"id": str(r[0]), "label": str(r[1])})
                    for record in records:
                        sv = record.get(rel_data["source_column"])
                        record[inc] = by_src.get(str(sv), []) if sv else []
                    ...
                    placeholders = ", ".join([f":sv{i}" for i in range(len(unique_values))])
                    where_parts = [f"{rel_data['target_column']} IN ({placeholders})"]
                    o_params: dict = {f"sv{i}": v for i, v in enumerate(unique_values)}
                    if rel_data["tenant_scope"] and effective_tenant:
                        where_parts.append("tenant_id = :tid")
                        o_params["tid"] = effective_tenant
                    where_sql = " AND ".join(where_parts)
                    q = text(
                        f"SELECT * FROM {target_table} "
                        f"WHERE {where_sql} LIMIT 1000"
                    )
                    rows = db.execute(q, o_params).fetchall()
                    by_src = {}
                    for row in rows:
                        src_val = str(row._mapping.get(rel_data["target_column"]))
                        if src_val not in by_src:
                            by_src[src_val] = []
                        cols = [c for c in row._mapping.keys() if c != "tenant_id"]
                        by_src[src_val].append({col: row._mapping[col] for col in cols})
                    for record in records:
                        sv = record.get(rel_data["source_column"])
                        record[inc] = by_src.get(str(sv), []) if sv else []
...
async def bulk_create_records(
    ...
    # Validate table and PK names (prevent SQL injection)
    _validate_identifier(table_name, "table_mapping")
    _validate_identifier(pk_col, "pk_column")
...
async def import_records(
    ...
    mode: str = Query("atomic", pattern="^(atomic|partial)$"),
