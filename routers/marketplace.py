...
async def list_items(item_type: str | None = None, is_featured: bool | None = None, is_free: bool | None = None, db: Session = Depends(get_db)):
    return {"status": "success", "data": MarketplaceEngine(db).list_items(item_type=item_type, is_featured=is_featured, is_free=is_free)}
...
async def apply(inst_id: str, body: dict, user: dict | None=None, db: Session = Depends(get_db)):
    result = MarketplaceEngine(db).apply_installation(user.get("tenant_id"), inst_id, body.get("project_id"))
    if not result["success"]:
