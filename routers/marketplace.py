...
async def list_items(item_type: Optional[str] = None,
...
async def get_item(item_code: str, db: Session = Depends(get_db)):
...
async def install(body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def apply(inst_id: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def list_installations(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def uninstall(item_code: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
