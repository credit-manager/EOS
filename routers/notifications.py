...
async def list_notifications(
    ...
    if notification_type:
        conditions.append("notification_type = :nt")
    ...
    rows = db.execute(
        text(
            f"SELECT id, tenant_id, user_id, channel, title, message, "
            f"notification_type, entity_code, record_id, event_id, "
            f"action_url, is_read, read_at, created_at "
            f"FROM dbp_notifications WHERE {where} "
    ...
    count_row = db.execute(
        text(f"SELECT COUNT(*) FROM dbp_notifications WHERE {where}"),
        params,
    ).fetchone()
    ...
    return {
        "status": "success",
        "data": notifications,
        "meta": {"count": count_row[0], "limit": limit, "offset": offset},
    }
...
async def get_notification(
    ...
    row = db.execute(
        text(
            "SELECT id, tenant_id, user_id, channel, title, message, "
            "notification_type, entity_code, record_id, event_id, "
            "action_url, is_read, read_at, extra_data, created_at "
            "FROM dbp_notifications WHERE id = :id"
        ),
        {"id": notification_id},
    ).fetchone()
...
async def mark_notification_read(
    ...
    ok = engine.mark_read(notification_id, user_id)

    if not ok:
        raise HTTPException(status_code=404, detail={
            "status": "error",
            "error": {"code": "NOT_FOUND", "message": "Notification not found or not yours"},
        })
...
async def mark_all_notifications_read(
    ...
    tenant_id = user.get("tenant_id")
    engine = NotificationEngine(db)
    count = engine.mark_all_read(user_id, tenant_id)
...
async def notifications_count(
    ...
    conditions = ["user_id = :uid", "is_read = false"]
...
async def delete_notification(
    ...
    result = db.execute(
        text("DELETE FROM dbp_notifications WHERE id = :id AND user_id = :uid"),
...
async def get_notification_preferences(
    ...
    rows = db.execute(
        text(
            ...
            "FROM dbp_notification_preferences "
            "WHERE user_id = :uid ORDER BY notification_type, channel"
        ),
        {"uid": user_id},
    ).fetchall()

    prefs = [
        {"id": r[0], "notification_type": r[1], "channel": r[2], "is_enabled": bool(r[3])}
        for r in rows
    ]
...
async def update_notification_preference(
    ...
    Body: {notification_type, channel, is_enabled}
    """
    user_id = user.get("id") or user.get("user_id")
    tenant_id = user.get("tenant_id")
    ...
    if not ntype:
        raise HTTPException(status_code=400, detail={
            "status": "error",
            "error": {"code": "MISSING_FIELD", "message": "notification_type required"},
        })

    existing = db.execute(
        text(
            "SELECT id FROM dbp_notification_preferences "
            "WHERE user_id = :uid AND notification_type = :nt AND channel = :ch"
        ),
        {"uid": user_id, "nt": ntype, "ch": channel},
    ).fetchone()

    if existing:
        db.execute(
            text(
                "UPDATE dbp_notification_preferences SET is_enabled = :ie "
                "WHERE id = :id"
            ),
            {"id": existing[0], "ie": is_enabled},
        )
    else:
        db.execute(
            text(
...
async def list_notification_templates(
    ...
    rows = db.execute(
        text(
            "SELECT id, code, channel, notification_type, "
            "title_template, message_template, event_type, is_active "
            "FROM dbp_notification_templates ORDER BY code"
        ),
    ).fetchall()
...
async def update_notification_template(
    ...
    existing = db.execute(
        text("SELECT id FROM dbp_notification_templates WHERE id = :id"),
        {"id": template_id},
    ).fetchone()
    ...
    if updates:
        db.execute(
            text(f"UPDATE dbp_notification_templates SET {', '.join(updates)} WHERE id = :id"),
            params,
        )
        db.commit()
