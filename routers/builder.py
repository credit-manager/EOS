...
def _err(status_code, code, message):
    return HTTPException(status_code, detail={"status": "error",
...
async def list_projects(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def get_project(pid: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not proj:
        raise _err(404, "NOT_FOUND", "Project not found")
...
async def update_settings(pid: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if not BuilderEngine(db).update_settings(user.get("tenant_id"), pid, body):
        raise _err(404, "NOT_FOUND", "Project not found")
    db.commit()
...
async def set_modules(pid: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    modules = body.get("modules")
    if not isinstance(modules, list):
        raise _err(400, "MISSING", "modules list required")
    result = BuilderEngine(db).set_modules(user.get("tenant_id"), pid, modules)
    if not result["success"]:
        raise _err(400, "MODULES_FAILED", result["error"])
    db.commit()
    return {"status": "success"}


@router.post("/projects/{pid}/entities", dependencies=[Depends(require_permission("dynamic", "create")), Depends(write_limiter.check)])
async def add_entity(pid: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    result = BuilderEngine(db).add_entity(user.get("tenant_id"), pid, body)
    if not result["success"]:
        raise _err(400, "ENTITY_FAILED", result["error"])
    db.commit()
    return {"status": "success", "data": result}
...
async def remove_entity(pid: str, ecode: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def add_relationship(pid: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    result = BuilderEngine(db).add_relationship(user.get("tenant_id"), pid, body)
    if not result["success"]:
        raise _err(400, "REL_FAILED", result["error"])
    db.commit()
    return {"status": "success"}


@router.put("/projects/{pid}/roles", dependencies=[Depends(require_permission("dynamic", "update")), Depends(write_limiter.check)])
async def set_roles(pid: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if not BuilderEngine(db).set_roles(user.get("tenant_id"), pid, body):
        raise _err(404, "NOT_FOUND", "Project not found")
    db.commit()
    return {"status": "success"}
...
async def add_workflow(pid: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    result = BuilderEngine(db).add_workflow(user.get("tenant_id"), pid, body)
    if not result["success"]:
        raise _err(400, "WF_FAILED", result["error"])
    db.commit()
    return {"status": "success"}


@router.post("/projects/{pid}/kpis", dependencies=[Depends(require_permission("dynamic", "create")), Depends(write_limiter.check)])
async def add_kpi(pid: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    result = BuilderEngine(db).add_kpi(user.get("tenant_id"), pid, body)
    if not result["success"]:
        raise _err(400, "KPI_FAILED", result["error"])
    db.commit()
    return {"status": "success"}


@router.get("/projects/{pid}/preview", dependencies=[Depends(require_permission("dynamic", "read")), Depends(read_limiter.check)])
async def preview(pid: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    preview_data = BuilderEngine(db).preview(user.get("tenant_id"), pid)
    if not preview_data:
        raise _err(404, "NOT_FOUND", "Project not found")
...
async def publish(pid: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not result["success"]:
        ...
        raise HTTPException(400, detail={"status": "error", "error": {
            "code": code, "message": result["error"], "details": result.get("validation")}})
    return {"status": "success", "data": result}


@router.get("/active", dependencies=[Depends(require_permission("dynamic", "read")), Depends(read_limiter.check)])
async def get_active(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    active = BuilderEngine(db).get_active_config(user.get("tenant_id"))
    if not active:
        raise _err(404, "NOT_FOUND", "No published configuration for this tenant")
...
async def list_versions(pid: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def rollback(pid: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    result = BuilderEngine(db).rollback(user.get("tenant_id"), pid, version_id,
