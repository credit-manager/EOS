...
async def create_journal_entry(company_id: str, body: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    jeid = AccountingEngine(db).create_journal_entry(_tenant(user), company_id, body["entry_date"], body["entry_type"], description=body.get("description"), reference=body.get("reference"), fiscal_year_id=body.get("fiscal_year_id"), created_by=user.get("id") or user.get("user_id"))
    if not jeid:
...
async def post_journal_entry(je_id: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    result = AccountingEngine(db).post_journal_entry(je_id, _tenant(user))
