...
def api(method, path, data=None, token="", idempotency_key=None):
    headers = {"Content-Type": "application/json"}
    if token:
        headers["Authorization"] = "Bearer " + token
    if method == "POST" and any(segment in path for segment in ("/transactions", "/bank-transfer", "/cash", "/links")):
        headers["Idempotency-Key"] = idempotency_key or f"p81-{uuid.uuid4().hex}"
...
# ═══════ GATEWAYS ═══════
print("--- Gateways ---")
gw1 = api("POST", "/payments/gateways", {"gateway_name": "Stripe", "gateway_type": "stripe", "config": {"publishable_key": "pk_test_xxx", "secret_key": "sk_test_xxx"}}, token)
test("81.1 Gateway created", "_err" not in gw1)
gw2 = api("POST", "/payments/gateways", {"gateway_name": "Mada", "gateway_type": "mada"}, token)
...
# ═══════ TRANSACTIONS ═══════
print("\n--- Transactions ---")
txn1 = api("POST", "/payments/transactions", {"amount": 5000, "currency": "SAR", "payment_method": "credit_card", "customer_id": "cust-001", "reference_type": "sales_order", "reference_id": "SO-001"}, token, "p81-txn-1")
test("81.4 Transaction created", "_err" not in txn1 and "transaction_id" in txn1.get("data", {}))
txn_id = txn1["data"]["transaction_id"]
txn2 = api("POST", "/payments/transactions", {"amount": 2500, "currency": "SAR", "payment_method": "bank_transfer", "customer_id": "cust-002"}, token, "p81-txn-2")
test("81.5 Second transaction created", "_err" not in txn2)
comp = api("POST", f"/payments/transactions/{txn_id}/complete", token=token, idempotency_key="p81-complete-1")
...
test("81.7 Transactions listable", "_err" not in txns and (txns.get("total", 0) >= 2 or len(txns.get("data", [])) >= 2))
...
# ═══════ REFUNDS ═══════
print("\n--- Refunds ---")
refund = api("POST", f"/payments/transactions/{txn_id}/refund", {"amount": 1000}, token, "p81-refund-1")
test("81.10 Refund created", "_err" not in refund and refund.get("data", {}).get("status") == "completed")

# ═══════ PAYMENT LINKS ═══════
print("\n--- Payment Links ---")
link = api("POST", "/payments/links", {"amount": 1500, "description": "Invoice INV-2026-001", "customer_email": "customer@example.com"}, token, "p81-link-1")
test("81.11 Payment link created", "_err" not in link and "payment_url" in link.get("data", {}))

# ═══════ BANK TRANSFER ═══════
print("\n--- Bank Transfer ---")
bt = api("POST", "/payments/bank-transfer", {"amount": 8000, "bank_name": "Al Rajhi Bank", "account_number": "SA1234567890", "reference": "TRF-2026-001"}, token, "p81-bank-1")
test("81.12 Bank transfer recorded", "_err" not in bt)

# ═══════ CASH ═══════
print("\n--- Cash Payment ---")
cash = api("POST", "/payments/cash?amount=500", token=token, idempotency_key="p81-cash-1")
...
# ═══════ FAIL TRANSACTION ═══════
print("\n--- Edge Cases ---")
fail_txn = api("POST", "/payments/transactions", {"amount": 100, "currency": "SAR", "payment_method": "credit_card"}, token, "p81-fail-create")
fail_id = fail_txn["data"]["transaction_id"]
fail_result = api("POST", f"/payments/transactions/{fail_id}/fail?reason=insufficient_funds", token=token, idempotency_key="p81-fail-1")
