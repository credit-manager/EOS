...
def test_entity_lookup_must_not_be_code_only():
    src = _source()
    assert '"SELECT id, table_mapping, tenant_id FROM dbp_entities WHERE code = :code"' in src
...
def test_create_cannot_trust_client_tenant_id():
    ...
    assert 'clean_data["tenant_id"] = effective_tenant' in src
...
def test_include_target_lookup_must_not_be_code_only():
    src = _source()
    assert '"SELECT table_mapping, tenant_id FROM dbp_entities WHERE code = :code"' in src
    assert "tenant_id" in src
