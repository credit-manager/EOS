...
export default function PlatformApp() {
  ...
  const authenticated = useCallback((data: Session) => {
    const next = { ...data, expires_at: Date.now() + data.expires_in * 1000 };
    localStorage.setItem(TOKEN_KEY, next.access_token);
    if (next.refresh_token) localStorage.setItem(REFRESH_TOKEN_KEY, next.refresh_token);
  ...
  const logout = useCallback(async () => {
    ...
    if (token) { try { await logoutSession(token); } catch {} }
