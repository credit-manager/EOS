...
type SessionUser = { id?:string; email?:string; tenant_id?:string; roles?:string[] };
...
function LoginScreen({ language, onLanguageChange, onLogin }: { language: Language; onLanguageChange: () => void; onLogin: (user: SessionUser) => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const user = response.data?.data?.user || await authAPI.getCurrentUser().then((r) => r.data?.data);
...
export default function App() {
  ...
  useEffect(() => {
    let alive = true;
    const handleExpired = () => { if (alive) setUser(null); };
    window.addEventListener('eos:auth-expired', handleExpired);
    if (!hasStoredSession()) {
      setSessionChecked(true);
      return () => { alive = false; window.removeEventListener('eos:auth-expired', handleExpired); };
    }
    authAPI.getCurrentUser()
      .then((response) => { if (alive) setUser(response.data?.data || null); })
      .catch(() => { if (alive) setUser(null); })
      .finally(() => { if (alive) setSessionChecked(true); });
    return () => { alive = false; window.removeEventListener('eos:auth-expired', handleExpired); };
  }, []);

  const logout = async () => { await authAPI.logout(); setUser(null); setPage('home'); setBuilder(false); };

  if (!sessionChecked) {
    return <div className="eos-auth-shell"><div className="eos-card eos-auth-card"><div className="eos-brand eos-auth-brand"><span className="eos-logo">E</span><span>EOS</span></div><p>{lang === 'ar' ? 'جارٍ التحقق من الجلسة…' : 'Checking session…'}</p></div></div>;
  }
  if (!user) return <LoginScreen language={lang} onLanguageChange={() => setLang(lang === 'ar' ? 'en' : 'ar')} onLogin={setUser} />;
  ...
  return <div className="eos-app" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
    <aside className="eos-sidebar">
      <div className="eos-brand"><span className="eos-logo">E</span><span>EOS</span></div>
      <button className="eos-create" onClick={() => setBuilder(true)}><FiPlus/><span>{t.onboarding}</span></button>
      <nav>{nav.map(([label,key,Icon]) => <button key={key} className={`eos-nav-item ${page===key?'is-active':''}`} onClick={() => { setBuilder(false); setPage(key); }}><Icon/><span>{label}</span></button>)}</nav>
      <div className="eos-sidebar-footer"><span className="eos-status-dot"/>{t.connected}</div>
    </aside>
    <main className="eos-main">
      <header className="eos-topbar">
        <button className="eos-icon-button eos-mobile-menu" aria-label={lang === 'ar' ? 'القائمة' : 'Menu'}><FiMenu/></button>
        ...
        <div className="eos-top-actions">
          ...
          <button className="eos-icon-button" aria-label={lang==='ar'?'الإشعارات':'Notifications'}><FiBell/></button>
          <div className="eos-avatar" title={user.email || 'EOS'}>{(user.email || 'EOS').slice(0, 2).toUpperCase()}</div>
          <button className="eos-icon-button" aria-label={lang === 'ar' ? 'تسجيل الخروج' : 'Log out'} onClick={logout}><FiLogOut/></button>
        </div>
      </header>
      <div className="eos-content">
        {builder?<EosOnboardingWizard language={lang} onBack={() => setBuilder(false)} onComplete={() => setBuilder(false)}/>:page==='customers'?<Customers t={t} language={lang}/>:page==='inventory'?<Inventory t={t} language={lang}/>:page==='sales'?<Sales language={lang}/>:page==='reports'?<Reports t={t} language={lang}/>:page==='settings'?<Settings t={t}/>:<Dashboard t={t} onBuild={() => setBuilder(true)} language={lang}/>}</div>
    </main>
  </div>;
}

function Dashboard({t,onBuild,language}:{t:Translation;onBuild:()=>void;language:Language}) {
  const [summary,setSummary]=useState<any>(null); const [sales,setSales]=useState<any>(null); const [inventory,setInventory]=useState<any>(null); const [loadError,setLoadError]=useState('');
  useEffect(() => { let alive=true; Promise.all([reportsAPI.profitAndLoss(),reportsAPI.sales(),reportsAPI.inventory()]).then(([p,s,i])=>{if(alive){setSummary(p.data?.data);setSales(s.data?.data);setInventory(i.data?.data);setLoadError('');}}).catch((error)=>{if(alive)setLoadError(readApiError(error,language==='ar'?'تعذر تحميل بيانات لوحة التحكم.':'Dashboard data could not be loaded.'));});return()=>{alive=false;}; },[language]);
